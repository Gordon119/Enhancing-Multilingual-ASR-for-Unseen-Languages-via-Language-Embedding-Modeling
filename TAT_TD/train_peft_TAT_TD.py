...
huggingface_user_token ='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'
...
def experiment(input_arg, model, processor, data_collator, repo_name, data_train, data_test, time, output_dir):
    ...
    # Push to Hub    
    peft_model_id = "Gordon119/" + f"TAT_TD-{input_arg['model_config']}-reproduce-mix".replace("/", "-")
    ...
    nlp2.write_csv(pred_results, f'{output_dir}/pred_{time}_epoch_{input_arg["specified_epoch"]}.csv')
...
def main(arg=None):
    ...
    if input_arg.get('checkpoint', None) is not None:
        ...
        # model = load_peft_model_from_hub("EricChang/TAT-TD-openai-whisper-large-v3-Lora-epoch5-total5epoch") 
    ...
    for i in range(input_arg['total_epoch']):
        ...
        print("===============================")
        model = experiment(input_arg, model, processor, data_collator, repo_name, data_train, data_test, time, output_dir=input_arg["output_dir"])
