...
def remove_successive_duplicates(s):
    result = ''
    prev_char = None
    for char in s:
        if char != prev_char:
...
def load_peft_model_from_hub(peft_model_id):
    ...
    model = PeftModel.from_pretrained(model, peft_model_id, token="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") 
...
def experiment(peft_model_id, input_arg, model, processor, data_collator, repo_name, data_train, data_test, time, output_dir):
    ################
    #     Train    #
    ################
    ...
    if not input_arg.get("only_eval", False):
        ...
        try:
            model.push_to_hub(peft_model_id, use_auth_token='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')
        except:
    ...
    for step, batch in enumerate(tqdm(eval_dataloader)):
        ...
        with torch.no_grad():
            ...
            # remove duplicate
            # print(pred_str)
            # pred_str = [remove_successive_duplicates(i) for i in pred_str]
            # print(pred_str)
    ...
    nlp2.write_csv(pred_results, f'{output_dir}/pred_{time}_{peft_model_id.replace("/", "-")}.csv')
...
def main(arg=None):
    ...
    input_arg["custom_set_train"] = "data/TD_TAT/train.csv"
    ...
    input_arg["custom_set_test"] = "data/TD_TAT/test.csv"
    ...
    models = [
        "Gordon119/TAT-openai-whisper-large-v2-mix-tag-epoch2-total5epoch",
    ]
    for peft_model_id in models:
        model = experiment(peft_model_id, input_arg, model, processor, data_collator, repo_name, data_train, data_test, time, output_dir=input_arg["output_dir"])
