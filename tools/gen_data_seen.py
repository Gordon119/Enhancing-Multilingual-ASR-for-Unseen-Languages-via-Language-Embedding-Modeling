...
for y, z in [("train", "transcript_1h_train"), ("val", "transcript_10min_dev"), ("test", "transcript_10min_test")]:
    ...
    for file in files:
        ...
        Path(f"data/seen_languages/{lang_code}").mkdir(parents=True, exist_ok=True)
    for key, val in dfs.items():
        if len(val) > 1:
            pd.concat(val).to_csv(f"data/seen_languages/{key}/{y}.csv", index=False)
        else:
            val[0].to_csv(f"data/seen_languages/{key}/{y}.csv", index=False)
    print(dfs.keys())
    # print(skip)
